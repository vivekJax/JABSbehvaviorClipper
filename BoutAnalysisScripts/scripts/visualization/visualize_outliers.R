...
tryCatch({ options(editor = "vim") }, error = function(e) { tryCatch({ options(editor = NULL) }, error = function(e2) BoutAnalysisScripts/scripts/visualization/visualize_outliers.R) })
# Fix R environment issues
options(editor = NULL)
...
utils_path <- file.path(script_dir, "..", "..", "utils", "data_preprocessing.R")
if (file.exists(utils_path)) {
  source(utils_path)
} else {
  # Try alternative path
  utils_path <- file.path(dirname(script_dir), "utils", "data_preprocessing.R")
  if (file.exists(utils_path)) {
    source(utils_path)
  }
}
