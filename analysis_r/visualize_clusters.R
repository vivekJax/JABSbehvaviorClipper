...
plot_cluster_heatmap <- function(X, labels, feature_names, output_path) {
  ...
  for (i in seq_along(unique_labels)) {
    ...
    cluster_means[i, ] <- colMeans(X[cluster_mask, , drop=FALSE])
  }
  
  # Select top features by variance across clusters
  feature_vars <- apply(cluster_means, 2, var)
  ...
  png(output_path, width=12, height=max(6, length(unique_labels)*0.5), units="in", res=300)
