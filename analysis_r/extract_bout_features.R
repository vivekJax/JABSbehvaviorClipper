      ...
      for (identity_id in names(labels)) {
        ...
        for (behavior_name in names(behaviors)) {
          ...
          if (is.data.frame(behavior_bouts)) {
            present_bouts <- behavior_bouts[behavior_bouts$present == TRUE, , drop=FALSE]
            
            for (i in seq_len(nrow(present_bouts))) {
              bout_counter <- bout_counter + 1
              all_bouts[[bout_counter]] <- list(
                bout_id = bout_counter - 1,
                video_name = video_name,
                identity = identity_id,
                start_frame = present_bouts$start[i],
                end_frame = present_bouts$end[i],
...
main <- function() {
  ...
  if (length(bouts) == 0) {
    stop(sprintf("No bouts found for behavior '%s'", opt$behavior))
  }
  
  cat(sprintf("Found %d bouts for behavior '%s'\n", length(bouts), opt$behavior))
